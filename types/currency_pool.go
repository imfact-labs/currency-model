...
type CurrencyPool struct {
	...
	stsmap map[CurrencyID]base.State
	cids   []CurrencyID
}
...
func (cp *CurrencyPool) Set(st base.State) error {
	cp.Lock()
	defer cp.Unlock()
...
func (cp *CurrencyPool) States() map[CurrencyID]base.State {
	cp.RLock()
	defer cp.RUnlock()

	m := map[CurrencyID]base.State{}
...
func (cp *CurrencyPool) State(cid CurrencyID) (base.State, bool) {
...
func (cp *CurrencyPool) TraverseState(callback func(cid CurrencyID, de base.State) bool) {
	cp.RLock()
	defer cp.RUnlock()
